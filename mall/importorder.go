...
func AddOrder(clt *core.Client, req *ImportOrderRequest, isHistory uint) (err error) {
	incompleteURL := util.StringsJoin("https://api.weixin.qq.com/mall/importorder?action=add-order&is_history=", strconv.Itoa(int(isHistory)), "&access_token=")
	var result struct {
		core.Error
	}
...
func UpdateOrder(clt *core.Client, req *ImportOrderRequest, isHistory uint) (err error) {
	incompleteURL := util.StringsJoin("https://api.weixin.qq.com/mall/importorder?action=update-order&is_history=", strconv.Itoa(int(isHistory)), "&access_token=")
	var result struct {
		core.Error
	}
	if err = clt.PostJSON(incompleteURL, req, &result); err != nil {
