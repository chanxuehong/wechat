...
func Refund2(clt *core.Client, req *RefundRequest) (resp *RefundResponse, err error) {
	...
	if n, err = strconv.ParseInt(m2["settlement_refund_fee"], 10, 64); err != nil {
		err = fmt.Errorf("parse settlement_refund_fee:%q to int64 failed: %s", m2["settlement_refund_fee"], err.Error())
		return
	} else {
		resp.SettlementRefundFee = n
	}
	...
	if n, err = strconv.ParseInt(m2["settlement_total_fee"], 10, 64); err != nil {
		err = fmt.Errorf("parse settlement_total_fee:%q to int64 failed: %s", m2["settlement_total_fee"], err.Error())
		return
	} else {
		resp.SettlementTotalFee = n
	}
