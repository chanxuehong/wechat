...
//  通常情况下一个 Server 实例用于处理一个component_verify_ticket的推送消息(事件), 此时建议指定 appId 用于约束消息(事件);
//  特殊情况下也可以一个 Server 实例用于处理多个component_verify_ticket的推送, 此时要求这些公众号的 token 是一样的, 并且 appId 必须设置为 "".
type Server struct {
	appId string
...
//  appId:        可选; AppId, 如果设置了值则安全模式时该Server只能处理 AppId 为该值的消息(事件);
//  token:        必须; 用于验证签名的token;
