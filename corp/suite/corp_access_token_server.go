...
//  NOTE:
//  1. 用于单进程环境.
//  2. 因为 CorpAccessTokenServer 同时也是一个简单的中控服务器, 而不是仅仅实现 AccessTokenServer 接口,
...
func NewCorpAccessTokenServer(suiteId string, suiteAccessTokenServer AccessTokenServer,
	authCorpId, permanentCode string, httpClient *http.Client) (srv *CorpAccessTokenServer) {
	...
	srv = &CorpAccessTokenServer{
		client: Client{
			SuiteId:           suiteId,
			AccessTokenServer: suiteAccessTokenServer,
...
func (srv *CorpAccessTokenServer) getToken() (token AccessTokenInfo, cached bool, err error) {
	srv.tokenGet.Lock()
