...
func aesEncryptMsg(random, rawXMLMsg []byte, CorpId string, AESKey [32]byte) (encryptedMsg []byte) {
	const BLOCK_SIZE = 32
...
func aesDecryptMsg(encryptedMsg []byte, CorpId string, AESKey [32]byte) (random [16]byte, rawXMLMsg []byte, err error) {
	const BLOCK_SIZE = 32
	...
	if CorpIdHave != CorpId { // crypto/subtle.ConstantTimeCompare ???
		err = fmt.Errorf("CorpId mismatch, have: %s, want: %s", CorpIdHave, CorpId)
		return
	}
