...
func CreateTemporaryQRCode(clt *core.Client, SceneId uint32, ExpireSeconds int) (qrcode *TemporaryQRCode, err error) {
	if SceneId == 0 {
...
func CreatePermanentQRCode(clt *core.Client, SceneId uint32) (qrcode *PermanentQRCode, err error) {
	if SceneId == 0 {
		err = errors.New("SceneId should be greater than 0")
		return
	}
...
func CreatePermanentQRCodeWithSceneString(clt *core.Client, SceneString string) (qrcode *PermanentQRCode, err error) {
	if SceneString == "" {
