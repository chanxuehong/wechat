...
func AddKfAccount(clt *core.Client, account, nickname, password string, isPwdPlain bool) (err error) {
	if password == "" {
...
//  password:   客服账号登录密码
//  isPwdPlain: 标识 password 是否为明文格式, true 表示是明文密码, false 表示是密文密码.
func SetKfAccount(clt *core.Client, account, nickname, password string, isPwdPlain bool) (err error) {
...
func UploadKfHeadImage(clt *core.Client, kfAccount, imagePath string) (err error) {
	if kfAccount == "" {
...
func UploadKfHeadImageFromReader(clt *core.Client, kfAccount, filename string, reader io.Reader) (err error) {
	if kfAccount == "" {
		return errors.New("empty kfAccount")
	}
	if filename == "" {
...
// 删除客服账号
func DeleteKfAccount(clt *core.Client, kfAccount string) (err error) {
	var result core.Error
