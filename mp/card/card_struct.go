...
type GeneralCoupon struct {
	CardBaseInfo  `json:"base_info"`
	DefaultDetail string `json:"default_detail"` // 描述文本
}
...
type Discount struct {
	CardBaseInfo `json:"base_info"`
	Discount     int `json:"discount"` // 折扣券专用，表示打折额度（百分比）。填30 就是七折。
}
...
type MemberCard struct {
	...
	SupplyBalance     bool   `json:"supply_balance"`              // 是否支持储值
	...
	Prerogative       string `json:"prerogative"`                 // 特权说明
...
type BoardingPass struct {
	CardBaseInfo  `json:"base_info"`
	From          string `json:"from"`                     // 起点，上限为18 个汉字
	To            string `json:"to"`                       // 终点，上限为18 个汉字
	Flight        string `json:"flight"`                   // 航班
...
type CardBaseInfo struct {
	...
	LogoURL     string `json:"logo_url"`            // 卡券的商户logo，尺寸为300*300。
	CodeType    string `json:"code_type"`           // code 码展示类型
	BrandName   string `json:"brand_name"`          // 商户名字,字数上限为12 个汉字。（填写直接提供服务的商户名， 第三方商户名填写在source 字段）
	Title       string `json:"title"`               // 券名，字数上限为9 个汉字。(建议涵盖卡券属性、服务及金额)
	SubTitle    string `json:"sub_title,omitempty"` // 可选; 券名的副标题，字数上限为18个汉字。
	Color       string `json:"color"`               // 券颜色。按色彩规范标注填写Color010-Color100
	Notice      string `json:"notice"`              // 使用提醒，字数上限为9 个汉字。（一句话描述，展示在首页，示例：请出示二维码核销卡券）
	Description string `json:"description"`         // 使用说明。长文本描述，可以分行，上限为1000 个汉字。
	...
	SKU struct {
		Quantity int `json:"quantity"` // 上架的数量。（不支持填写0或无限大）
	} `json:"sku"` // 商品信息
