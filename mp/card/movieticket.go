...
type MovieTicketUpdateUserParameters struct {
	Code   string `json:"code"`              // 电影票的序列号。
	CardId string `json:"card_id,omitempty"` // 电影票card_id。自定义code 的电影票为必填，非自定义code 的电影票不必填。

	TicketClass   string `json:"ticket_class,omitempty"`   // 电影票的类别，如2D、3D
	ShowTime      int64  `json:"show_time,omitempty"`      // 电影放映时间对应的时间戳。
	Duration      int    `json:"duration,omitempty"`       // 放映时长，填写整数
	ScreeningRoom string `json:"screening_room,omitempty"` // 该场电影的影厅信息
