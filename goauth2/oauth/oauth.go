...
func (t *Transport) Exchange(code string) (*Token, error) {
	...
	// If the transport has a token, preserve existing refresh token.
	tok := t.Token
	if tok == nil {
		tok = new(Token)
	}
	...
	req, err := http.NewRequest("GET", t.TokenURL, strings.NewReader(v.Encode()))
	if err != nil {
		return nil, err
	}
	...
	if resp.StatusCode != 200 {
		return nil, &OAuthError{"Exchange", resp.Status}
	}

	var result struct {
		AccessToken  string        `json:"access_token"`  // 网页授权接口调用凭证,注意：此access_token与基础支持的access_token不同
		RefreshToken string        `json:"refresh_token"` // 用户刷新access_token
		ExpiresIn    time.Duration `json:"expires_in"`    // access_token接口调用凭证超时时间，单位（秒）
	...
	result.ExpiresIn *= time.Second
	if result.ExpiresIn == 0 {
		tok.Expiry = time.Time{}
	} else {
		tok.Expiry = time.Now().Add(result.ExpiresIn)
	}
...
func (t *Transport) Refresh() error {
	...
	client := &http.Client{Transport: t.transport()}
	req, err := http.NewRequest("GET", t.RefreshTokenURL, strings.NewReader(v.Encode()))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	...
	if resp.StatusCode != 200 {
		return &OAuthError{"Refresh", resp.Status}
	}
	...
	t.Token.AccessToken = result.AccessToken
	// Don't overwrite `RefreshToken` with an empty value
	if len(result.RefreshToken) > 0 {
		t.Token.RefreshToken = result.RefreshToken
	}
	result.ExpiresIn *= time.Second
	if result.ExpiresIn == 0 {
		t.Token.Expiry = time.Time{}
	} else {
		t.Token.Expiry = time.Now().Add(result.ExpiresIn)
	}
	t.Token.OpenId = result.OpenId
	t.Token.Scope = result.Scope
	return nil
}
