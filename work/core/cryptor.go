...
func AESEncryptMsg(random, rawXMLMsg []byte, receiveId string, aesKey []byte) (ciphertext []byte) {
	...
	receiveIdOffset := 20 + len(rawXMLMsg)
	contentLen := receiveIdOffset + len(receiveId)
	...
	copy(plaintext[receiveIdOffset:], receiveId)
...
func AESDecryptMsg(ciphertext []byte, aesKey []byte) (random, rawXMLMsg, received []byte, err error) {
	...
	receivedOffset := 20 + rawXMLMsgLen
	if len(plaintext) <= receivedOffset {
		err = fmt.Errorf("msg length too large: %d", rawXMLMsgLen)
		return
	}

	random = plaintext[:16:20]
	rawXMLMsg = plaintext[20:receivedOffset:receivedOffset]
	received = plaintext[receivedOffset:]
	return
}
